...
describe('object-resolve-path', function() {
  ...
  it('should strip a leading dot', function(){
    expect(resolve(testObj, '.a')).to.equal('b');
    expect(resolve(testObj, '.')).to.equal(undefined);
  });
  it('should return undefined when path does not exist', function() {
    expect(resolve(testObj, 'x')).to.equal(undefined);
  });

  it('should fail to get a path when it contains brackets inside', function(){
    expect(resolve(testObj, "hy[ph]en")).to.equal(undefined); //a flaw by design, we would have to parse the whole tree if we would like to properly discern these
  });
